...
export class Sample extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save Sample entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("Sample", id.toString(), this);
    }
  }

  static load(id: string): Sample | null {
    return changetype<Sample | null>(store.get("Sample", id));
  }
